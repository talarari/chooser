...
It needs real network egress to the TURN server, which **hermetic/locked-down CI usually lacks**. This repo's CI allowlists egress (DNS over UDP resolves, but STUN/TURN flows are filtered — STUN-over-UDP gets no response and raw TURN-over-TCP/TLS on 443 never completes its allocation), so the relay can't be reached. So the test **probes for a relay candidate and skips — rather than fails — when TURN is unreachable**, validating the relay path only where connectivity exists (a dev machine, a permissive CI). Because of that, and because WebKit-on-Linux ≠ Safari, the Open Relay TURN fallback can't be fully proven in CI; **final confirmation is a real iPhone Safari ↔ Chrome session** on the same and on different networks (issue #2 acceptance criteria).
